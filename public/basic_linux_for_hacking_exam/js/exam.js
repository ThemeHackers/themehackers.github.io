document.addEventListener('DOMContentLoaded', function () {
    ...
    fetch('db/basic_linux_for_hacking_exam.json')
        .then(response => response.json())
        .then(questions => startExam(questions))
        .catch(err => {
            const container = document.getElementById('exam-container');
            container.innerHTML = '<div class="th-alert th-alert-danger">Failed to load exam. Please try again later.</div>';
        });
    ...
    function startExam(questions) {
        ...
        const sessionKey = 'linux_exam_state';

        const saved = sessionStorage.getItem(sessionKey);
        if (saved) {
            try {
                const state = JSON.parse(saved);
                if (Array.isArray(state.userAnswers) && typeof state.current === 'number' && typeof state.score === 'number') {
        ...
        function showQuestion() {
                    ...
                    <form id="question-form">
                        ${q.options.map((opt, i) => `
                            <label class="th-choice-label" data-index="${i}">
                                <input type="radio" class="th-choice-radio" name="option" value="${i}" style="vertical-align:middle;"> <b>${String.fromCharCode(65 + i)}</b>. ${opt}
                            </label>
                        `).join('')}
                        <div style="display:flex;gap:1rem;margin-top:1.2rem;">
                            <button type="button" id="back-btn" class="th-btn-outline-primary" style="width:40%;font-size:1.1rem;" ${current === 0 ? 'disabled' : ''}>Back</button>
                            <button type="submit" class="th-btn-primary" style="width:60%;font-size:1.1rem;">${current === questions.length-1 ? 'Finish' : 'Next'}</button>
                        </div>
                    </form>
                </div>
            `;
            ...
            if (userAnswers[current] !== undefined) {
                ...
                const radio = container.querySelector(`input[name="option"][value="${prev}"]`);
                if (radio) {
                    radio.checked = true;
                    labels[prev].classList.add('selected');
                }
            }
            ...
            document.getElementById('question-form').onsubmit = function (e) {
                ...
                userAnswers[current] = selectedValue;
            ...
            Object.entries(sections).forEach(([section, qs], sidx) => {
                ...
                qs.forEach((q, idx) => {
                    ...
                    q.options.forEach((opt, i) => {
                        ...
                        html += `<div class="${classes}"><span class="th-review-icon">${icon}</span> <b>${String.fromCharCode(65 + i)}</b>. ${opt}</div>`;
                    });
                    html += `</div></div>`;
                });
                html += `</div>`;
